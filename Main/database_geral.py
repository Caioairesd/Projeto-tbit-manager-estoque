...
def registrar_produto_db(nome, descricao, quantidade, valor):
    conn = get_connection()
    cursor = conn.cursor()
...
def atualizar_produto_db(nome, descricao, quantidade, valor):
    conn = get_connection()
    cursor = conn.cursor()
...
def registrar_funcionario_db(nome, DataDeNascimento, DataDeAdmissao,  CPF, Cidade, UF, Telefone, Email, Usuario, Senha):
